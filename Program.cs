...
app.MapPut("hotels", async ([FromBody] Hotel hotel, HotelDb db) => 
    {
        var hoteFromlDb = await db.Hotels.FindAsync(new object[] {hotel.Id});
        if(hoteFromlDb == null) return Results.NotFound("Hotel Not Found for Modification");
        hoteFromlDb.Latitude = hotel.Latitude;
        hoteFromlDb.Longitude = hotel.Longitude;
        hoteFromlDb.Name = hotel.Name;
        await db.SaveChangesAsync();
        return Results.NoContent();
    });
app.MapDelete("hotels/{id}", async (int id, HotelDb db) => {
    var hoteFromlDb = await db.Hotels.FindAsync(new object[] {id});
        if(hoteFromlDb == null) return Results.NotFound("Hotel Not Found for Deletion");
        db.Hotels.Remove(hoteFromlDb);
        await db.SaveChangesAsync();
        return Results.NoContent();
});
